 ...
 * the source rather than using the source directly.
 */
public class TranslationLookupTest {
	...
	public void testCanLookupPropertyOnTranslate() {
		PostDto postDto = new PostDto( "Saving Money through Extreme Couponing", new AuthorDto( 38, "Penelope Williams", 6 ) );
		Post post = Translate.to( Post.class ).withVariable( "userDao", userDao ).from( postDto );
	...
	public void testCanThrowMissingSourcePropertyExceptionOnLookupFailure() {
		PostDto postDto = new PostDto( "Love on the High Seas", new AuthorDto( 45, "Saul Madison", 1 ) );
		Translate.to( Post.class ).withVariable( "userDao", userDao ).from( postDto );
	}
	...
	public static class Post {
		...
		@Property( source = "userDao.getUser(author.id)" )
		private User author;
	...
	public static class UserDao {
		...
		public User getUser( int id ) {
			if ( usersByKey.containsKey( id ) ) {
				...
				throw new IllegalArgumentException( "Cannot find user corresponding to key " + id );
			}
		}
	}
